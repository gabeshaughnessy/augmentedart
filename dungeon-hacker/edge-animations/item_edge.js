/**
 * Adobe Edge: symbol definitions
 */
(function($, Edge, compId){
//images folder
var im='images/';

var fonts = {};
var opts = {
    'gAudioPreloadPreference': 'auto',

    'gVideoPreloadPreference': 'auto'
};
var resources = [
];
var symbols = {
"stage": {
    version: "4.0.0",
    minimumCompatibleVersion: "4.0.0",
    build: "4.0.0.359",
    baseState: "Base State",
    scaleToFit: "none",
    centerStage: "none",
    initialState: "Base State",
    gpuAccelerate: false,
    resizeInstances: false,
    content: {
            dom: [
            {
                id: 'item-card',
                type: 'image',
                rect: ['0', '0','500px','800px','auto', 'auto'],
                fill: ["rgba(0,0,0,0)",im+"item-card.jpg",'0px','0px']
            },
            {
                id: 'Item-Title',
                type: 'text',
                rect: ['4px', '402px','500px','68px','auto', 'auto'],
                text: "Item Title",
                align: "left",
                font: ['\'Lucida Console\', Monaco, monospace', 24, "rgba(0,0,0,1)", "900", "none", "normal"]
            },
            {
                id: 'Item-Description',
                type: 'text',
                rect: ['37px', '447px','440px','101px','auto', 'auto'],
                text: "Item Description goes here.",
                align: "left",
                font: ['\'Lucida Console\', Monaco, monospace', 18, "rgba(0,0,0,1)", "400", "none", "normal"]
            },
            {
                id: 'Item-Attributes',
                type: 'rect',
                rect: ['25px', '307px','457px','68px','auto', 'auto'],
                fill: ["rgba(192,192,192,0.00)"],
                stroke: [0,"rgb(0, 0, 0)","none"]
            },
            {
                id: 'AttributesLabel',
                type: 'text',
                rect: ['53px', '611px','133px','57px','auto', 'auto'],
                text: "Improves your:",
                align: "center",
                font: ['Lucida Console, Monaco, monospace', 15, "rgba(0,0,0,1)", "700", "none", ""]
            },
            {
                id: 'Equip-Item-Button',
                type: 'rect',
                rect: ['35px', '386px','445px','101px','auto', 'auto'],
                borderRadius: ["10px", "10px", "10px", "10px"],
                fill: ["rgba(192,192,192,1)"],
                stroke: [0,"rgba(0,0,0,1)","none"],
                c: [
                {
                    id: 'Equip-Button-text',
                    type: 'text',
                    rect: ['8px', '184px','387px','68px','auto', 'auto'],
                    text: "Tap to buy this Item",
                    align: "left",
                    font: ['Lucida Console, Monaco, monospace', 18, "rgba(0,0,0,1)", "400", "none", "normal"]
                }]
            }],
            symbolInstances: [

            ]
        },
    states: {
        "Base State": {
            "${_Item-Description}": [
                ["style", "top", '517px'],
                ["style", "text-align", 'left'],
                ["style", "height", '84px'],
                ["style", "font-size", '16px'],
                ["style", "left", '53px'],
                ["style", "width", '387px']
            ],
            "${_Item-Title}": [
                ["style", "top", '483px'],
                ["style", "text-align", 'left'],
                ["style", "width", '395px'],
                ["style", "font-weight", '900'],
                ["style", "left", '53px'],
                ["style", "font-size", '20px']
            ],
            "${_Equip-Item-Button}": [
                ["style", "top", '474px'],
                ["style", "height", '260px'],
                ["color", "background-color", 'rgba(192,192,192,0.00)'],
                ["style", "left", '45px'],
                ["style", "width", '409px']
            ],
            "${_AttributesLabel}": [
                ["style", "top", '582px'],
                ["style", "width", '254px'],
                ["style", "text-align", 'center'],
                ["style", "font-weight", 'bold'],
                ["style", "height", '57px'],
                ["style", "font-family", '\'Lucida Console\', Monaco, monospace'],
                ["style", "left", '136px'],
                ["style", "font-size", '14px']
            ],
            "${_Item-Attributes}": [
                ["color", "background-color", 'rgba(192,192,192,0.00)'],
                ["style", "height", '47px'],
                ["style", "top", '611px'],
                ["style", "left", '233px'],
                ["style", "width", '43px']
            ],
            "${_Equip-Button-text}": [
                ["style", "top", '184px'],
                ["style", "text-align", 'left'],
                ["style", "width", '387px'],
                ["style", "height", '68px'],
                ["style", "font-family", 'Lucida Console, Monaco, monospace'],
                ["style", "left", '8px'],
                ["style", "font-size", '18px']
            ],
            "${_Stage}": [
                ["color", "background-color", 'rgba(255,255,255,0.00)'],
                ["style", "width", '500px'],
                ["style", "height", '800px'],
                ["style", "overflow", 'hidden']
            ]
        }
    },
    timelines: {
        "Default Timeline": {
            fromState: "Base State",
            toState: "",
            duration: 250,
            autoPlay: true,
            labels: {
                "selected": 250
            },
            timeline: [
                { id: "eid116", tween: [ "style", "${_Item-Title}", "width", '395px', { fromValue: '395px'}], position: 0, duration: 0 },
                { id: "eid163", tween: [ "style", "${_Item-Description}", "top", '517px', { fromValue: '517px'}], position: 0, duration: 0 },
                { id: "eid157", tween: [ "style", "${_Equip-Item-Button}", "left", '45px', { fromValue: '45px'}], position: 0, duration: 0 },
                { id: "eid147", tween: [ "style", "${_AttributesLabel}", "left", '136px', { fromValue: '136px'}], position: 0, duration: 0 },
                { id: "eid154", tween: [ "style", "${_Equip-Item-Button}", "width", '409px', { fromValue: '409px'}], position: 0, duration: 0 },
                { id: "eid123", tween: [ "style", "${_Item-Description}", "left", '53px', { fromValue: '53px'}], position: 0, duration: 0 },
                { id: "eid142", tween: [ "style", "${_Item-Attributes}", "width", '43px', { fromValue: '43px'}], position: 0, duration: 0 },
                { id: "eid164", tween: [ "style", "${_AttributesLabel}", "top", '582px', { fromValue: '582px'}], position: 0, duration: 0 },
                { id: "eid153", tween: [ "style", "${_Item-Description}", "width", '387px', { fromValue: '387px'}], position: 0, duration: 0 },
                { id: "eid141", tween: [ "style", "${_Item-Attributes}", "height", '47px', { fromValue: '47px'}], position: 0, duration: 0 },
                { id: "eid161", tween: [ "style", "${_Item-Title}", "top", '483px', { fromValue: '483px'}], position: 0, duration: 0 },
                { id: "eid158", tween: [ "style", "${_Equip-Item-Button}", "top", '474px', { fromValue: '474px'}], position: 0, duration: 0 },
                { id: "eid162", tween: [ "style", "${_Item-Title}", "font-size", '20px', { fromValue: '20px'}], position: 0, duration: 0 },
                { id: "eid165", tween: [ "style", "${_Item-Attributes}", "top", '611px', { fromValue: '611px'}], position: 0, duration: 0 },
                { id: "eid90", tween: [ "color", "${_Equip-Item-Button}", "background-color", 'rgba(192,192,192,0.00)', { animationColorSpace: 'RGB', valueTemplate: undefined, fromValue: 'rgba(192,192,192,0.00)'}], position: 0, duration: 0 },
                { id: "eid148", tween: [ "style", "${_Item-Attributes}", "left", '233px', { fromValue: '233px'}], position: 0, duration: 0 },
                { id: "eid89", tween: [ "style", "${_Item-Description}", "height", '84px', { fromValue: '84px'}], position: 0, duration: 0 },
                { id: "eid114", tween: [ "style", "${_Item-Description}", "font-size", '16px', { fromValue: '16px'}], position: 0, duration: 0 },
                { id: "eid159", tween: [ "style", "${_Equip-Item-Button}", "height", '260px', { fromValue: '260px'}], position: 0, duration: 0 },
                { id: "eid115", tween: [ "style", "${_Item-Title}", "left", '53px', { fromValue: '53px'}], position: 0, duration: 0 },
                { id: "eid144", tween: [ "style", "${_AttributesLabel}", "width", '254px', { fromValue: '254px'}], position: 0, duration: 0 }            ]
        }
    }
},
"item-image": {
    version: "4.0.0",
    minimumCompatibleVersion: "4.0.0",
    build: "4.0.0.359",
    baseState: "Base State",
    scaleToFit: "none",
    centerStage: "none",
    initialState: "Base State",
    gpuAccelerate: false,
    resizeInstances: false,
    content: {
            dom: [
                {
                    id: 'image',
                    type: 'image',
                    rect: ['0px', '0px', '400px', '400px', 'auto', 'auto'],
                    fill: ['rgba(0,0,0,0)', 'images/default-item.png', '0px', '0px']
                }
            ],
            symbolInstances: [
            ]
        },
    states: {
        "Base State": {
            "${_image}": [
                ["style", "left", '0px'],
                ["style", "top", '0px']
            ],
            "${symbolSelector}": [
                ["style", "height", '400px'],
                ["style", "width", '400px']
            ]
        }
    },
    timelines: {
        "Default Timeline": {
            fromState: "Base State",
            toState: "",
            duration: 0,
            autoPlay: true,
            timeline: [
            ]
        }
    }
}
};


Edge.registerCompositionDefn(compId, symbols, fonts, resources, opts);

/**
 * Adobe Edge DOM Ready Event Handler
 */
$(window).ready(function() {
     Edge.launchComposition(compId);
});
})(jQuery, AdobeEdge, "item");
